...
class ForecastView(APIView):
    ...
    def get(self, request):
        ...
        if model_name == 'prophet':
            forecast = run_prophet_forecast(data)
        elif model_name == 'arima':
            forecast = run_arima_forecast(data)
        elif model_name == 'rf':
            forecast = run_rf_forecast(data)
        elif model_name == 'linear':
            forecast = run_linear_regression(data)
        elif model_name == 'lstm':
            forecast = run_lstm_forecast(data)
        else:
            return Response({'error': 'Неизвестная модель'}, status=400)

        return Response({'resource': resource, 'model': model_name,'city': city or 'Все','forecast': forecast})
